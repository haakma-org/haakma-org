  ...
  post {
    success {
      slackSend channel: '#backup',
                color: 'good',
                mrkdwn: true,
                message: "The backup <${env.PROJECT_URL}|${env.JOB_NAME}> | _#${env.BUILD_NUMBER}_ completed successfully."
    }
    failure {
      slackSend channel: '#backup',
                color: 'red',
                mrkdwn: true,
                message: "The backup <${env.PROJECT_URL}|${env.JOB_NAME}> | _#${env.BUILD_NUMBER}_ has failed."
    }
  }
}
