  ...
  post {
    success {
      slackSend channel: '#backup',
                color: 'good',
                mrkdwn: true,
                message: "The backup [${env.JOB_NAME](${env.PROJECT_URL}) | _#${env.BUILD_NUMBER}_ completed successfully."
    }
    failure {
      slackSend channel: '#backup',
                color: 'red',
                mrkdwn: true,
                message: "The backup [${env.JOB_NAME](${env.PROJECT_URL}) | _#${env.BUILD_NUMBER}_ has failed."
    }
  }
}
